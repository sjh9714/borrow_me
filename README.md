...
**결과** — 상품 100개 기준 **201회 → 3회** 쿼리 감소. k6 부하 테스트(30 VU, 30초): **p95 20ms, 256 req/s, 에러율 0%**
...
**결과** — 수정 전: 재고 50개에 100건 전부 성공(불일치). 수정 후 k6 재검증: **성공 50건 + 실패 50건, 최종 재고 0 (정합성 100%)**
...
|--------|-----|------|-------------|--------|------|
| **동시 예약** | 100 | 재고 50개, 1회씩 동시 요청 | 233ms | - | 성공 50건, 실패 50건, 초과 0건 |
| **상품 목록 조회** | 30 | 상품 100개 + 해시태그 + 팔로우, 30초 | 20ms | 256 req/s | 에러율 0%, 7,761회 처리 |
| **검색** | 30 | 다양한 검색어, 30초 | 72ms | 223 req/s | 에러율 0%, 6,754회 처리 |
