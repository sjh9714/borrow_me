...
이 문서는 README에 남아 있는 상품 목록 조회 개선 기록과 현재 clean repeat3 local k6 snapshot을 분리해
설명합니다. 원본 README의 p95 `23ms`는 raw artifact가 없는 historical original record이고, p95
`358.1088ms`는 현재 clean commit에서 보존한 local k6 snapshot입니다.
...
| 처리량 | 253 req/s |
| DB 쿼리 | 3회 |
| 증거 분류 | 원본 README 기록, raw k6 artifact 없음, 현재 측정 claim 아님 |
...
2026-05-23에 `main` clean commit 기준으로 product-listing clean repeat3 시나리오를 다시 실행해 raw
artifact와 derived summary metadata를 보존했습니다. 이 snapshot은 현재 실행 환경에서 k6 threshold를 통과한
local k6 결과이며, 원본 README의 After p95 23ms 기록을 대체하거나 같은 조건으로 재현했다고 주장하지 않습니다.

| 항목 | 값 |
| --- | --- |
| 시나리오 | `product-listing`, 30 VU, 30초 |
| 앱/DB | local profile app on `localhost:5001`, throwaway Docker MySQL `shop` on host port 3307, `k6/setup-data.sql` fixture |
| 실행 commit | `01c255a9f1863ef8f0b80854cc159a70eee036eb` |
| 실행 전 git 상태 | clean |
| p50 응답 시간 | 121.6ms |
| p95 응답 시간 | 358.1088ms |
| p99 응답 시간 | 557.66ms |
| HTTP 요청 처리량 | 115.72 req/s |
| HTTP 실패율 | 0.00% |
| checks | 10,683 / 10,683 성공 |
| query count | k6 artifact에는 없음. query-count guard는 `ProductQueryTest`에서 별도 검증 |
| raw artifact | `docs/evidence/k6/20260523T004642Z-product-listing/` |
| 증거 분류 | 현재 clean repeat3 local k6 snapshot, raw artifact 보존 |

## p95 23ms와 358.1088ms가 다른 이유

두 값은 같은 benchmark의 반복 결과가 아니라 증거 범주가 다릅니다. `23ms`는 원본 README에 남아 있던
After 기록으로, 당시 raw k6 output, 실행 환경, dataset artifact가 repository에 남아 있지 않습니다.
`358.1088ms`는 2026-05-23 clean commit과 `k6/setup-data.sql` fixture에서 다시 실행한 local k6
snapshot입니다. 따라서 이 문서에서는 `23ms`를 과거 개선 기록으로, `358.1088ms`를 현재 local snapshot으로
분리하고 둘을 직접 비교해 regression이나 현재 성능 개선 폭으로 주장하지 않습니다.
...
- 2026-05-23 clean repeat3 snapshot은 현재 실행 환경과 fixture 기준의 단일 local run이며, 운영 성능 claim으로
  확장하지 않습니다. p95 358.1088ms는 원본 README After p95 23ms와 별도 현재 snapshot 값입니다.
- 2026-05-22 artifact metadata에는 당시 `git status`가 포함되어 있으며 dirty worktree snapshot입니다.
...
| p95 개선을 어떻게 해석해야 하나요? | 운영 성능 주장이 아니라 원본 README에 기록된 Before/After 상대 개선입니다. 현재 clean repeat3 p95 358.1088ms는 별도 local k6 snapshot으로만 설명합니다. |
