...
이 문서는 README에 남아 있는 상품 목록 조회 개선 기록과 현재 로컬 remeasurement snapshot을 분리해
설명합니다.
...
재측정 결과이며, 원본 README의 After p95 23ms 기록을 대체하거나 같은 조건으로 재현했다고 주장하지 않습니다.
...
- 2026-05-23 clean repeat3 snapshot은 현재 실행 환경과 fixture 기준의 단일 local run이며, 운영 성능 claim으로
  확장하지 않습니다. p95 358.1ms는 원본 README After p95 23ms와 별도 현재 재측정 값입니다.
...
| p95 개선을 어떻게 해석해야 하나요? | 운영 성능 주장이 아니라 원본 README에 기록된 Before/After 상대 개선입니다. 현재 clean repeat3 p95 358.1ms는 별도 재측정 snapshot으로만 설명합니다. |
