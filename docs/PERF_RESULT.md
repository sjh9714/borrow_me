...
| 항목 | 상태 | 문서 |
| --- | --- | --- |
| 상품 목록 p95 / 처리량 / 쿼리 수 | 원본 README historical record + 2026-05-23 clean repeat3 local k6 snapshot + 2026-05-22 이전 snapshot | [PRODUCT_LIST_PERF.md](PRODUCT_LIST_PERF.md) |
...
- 위 원본 README 기록은 raw k6 artifact가 없는 historical original record이며, 현재 measured claim이나 현재 raw artifact가 아닙니다.
- 2026-05-23 product-listing clean repeat3 k6 재실행은 clean commit과 `k6/setup-data.sql` fixture에서의 current local snapshot입니다.
- product-listing p95 358.1088ms는 clean repeat3 local k6 snapshot 값이며, 원본 README After p95 23ms와 같은 조건의 재현값이 아닙니다.
...
동일한 조건의 재현이라고 주장하지 않습니다. 면접에서는 `23ms`와 `358.1088ms`를 직접 비교하지 않고,
각각 historical original record와 clean repeat3 local k6 snapshot으로 분리합니다.
