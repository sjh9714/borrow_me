...
상품 목록 N+1 개선, 예약 재고 정합성, 알림 흐름을 분리해 설명합니다. 상품 목록 p95 `23ms`는 원본
README historical record이고, 현재 보존된 clean repeat3 local k6 snapshot은 p95 `358.1088ms`입니다.
현재 자동 검증은 query-count guard와 reservation concurrency test입니다.
...
| p95 1,010ms -> 23ms를 지금 재현할 수 있나요? | 아닙니다. `23ms`는 raw artifact가 없는 원본 README historical record입니다. 현재 raw artifact가 있는 상품 목록 값은 2026-05-23 clean repeat3 local k6 snapshot p95 `358.1088ms`이며, 두 값은 직접 비교하지 않습니다. |
...
- clean repeat3 p95 `358.1088ms`를 원본 README After p95 `23ms`의 재현 또는 대체 수치처럼 말하지 않습니다.
